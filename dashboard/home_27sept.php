...
// error_reporting(E_ALL);
    ...
    try {
        ...
        if ($_SESSION['userrole'] == 'admin') {
            ...
            $sql = $db->prepare("SELECT * FROM `bowlersreleased` WHERE `approved` = :approved AND `release_status` = :release_status AND `removedby` != 'Admin' ORDER BY id DESC");
        ...
        if ($_SESSION['userrole'] != 'admin' || $_SESSION['userrole'] != 'admin') {
            ...
            if($seasonTourYearFlag == null){
                ...
                $sql = $db->prepare("SELECT * FROM `bowlerdataseason` WHERE `bowlerid` = '$bowlerUBAID'");
                $sql_avg = $db->prepare("SELECT * FROM `bowlerdataseason` WHERE `bowlerid` = '$bowlerUBAID' AND YEAR(eventdate) BETWEEN '$nextYear' AND '$currentYear' AND year='$nextYear/$year'");
            }else{
        	...
        	foreach ($dataFetchedSeasonTour_avg as $bowler) {
				...
				$sectValue1 = date('Y-m-d ', strtotime('09-01-2022'));
				$sectValue2 = date('Y-m-d ', strtotime('09-01-2023'));
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <!--button type="button" onclick="saveAndAddToTheRoster()" class="btn btn-success" >Yes, But approve the bowler transfer only</button-->
            ...
            <table id="released_bowlers_table_home" class="display">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if($singleScoreData['currentstatus']=='Suspended'){ echo 'Released Bowlers';} else{ echo $singleScoreData['team']; }?>
                        </td>
                        <td>
