...
if(isset($_GET['exportType'])){
    $sql = "SELECT `teamname`, `president`, `owner` FROM `teams` WHERE 1=1 $searchQuery ORDER BY $orderColumn $orderDir";
    $stmt = $db->prepare($sql);
    if (!empty($searchValue)) {
        $stmt->bindParam(':searchValue', $searchValue);
    }
    ...
    $sql = "SELECT `teamname`, `president`, `owner` FROM `teams` WHERE 1=1 $searchQuery ORDER BY $orderColumn $orderDir LIMIT :start, :limit";
    $stmt = $db->prepare($sql);
    if (!empty($searchValue)) {
        $stmt->bindParam(':searchValue', $searchValue);
    }
...
foreach ($dataFetched as $singleScoreData) {
    ...
    $row['no'] = $i++ ?? '-'; 
    $row['team'] = $singleScoreData['teamname'] ?? '-';
    $row['president'] = $presidentBowler['name'] ?? '-';
    $row['owner'] = $singleScoreData['owner'] ?? '-';
    $data[] = $row;
}
...
function exportCSV($data) {
    ...
    header('Content-Disposition: attachment; filename="team_officials.csv"');
    $output = fopen("php://output", "w");
    fputcsv($output, array('Team', 'President', 'Owner'));
    foreach ($data as $row) {
        fputcsv($output, $row);
    }
    fclose($output);
...
function exportExcel($data) {
    ...
    $sheet->setCellValue('C1', 'President');
    ...
    foreach ($data as $row) {
        ...
        $sheet->setCellValue('C' . $rowNum, $row['president']);
        $sheet->setCellValue('D' . $rowNum, $row['owner']);
        $rowNum++;
    }
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment; filename="team_officials.xlsx"');
...
function exportPDF($data) {
    ...
    // Set column headers
    $pdf->Cell(20, 10, 'No', 1);
    $pdf->Cell(70, 10, 'Team', 1);
    $pdf->Cell(50, 10, 'President', 1);
    $pdf->Cell(50, 10, 'Owner', 1);
    $pdf->Ln();
    ...
    foreach ($data as $row) {
        $pdf->Cell(20, 10, $i++, 1);
        $pdf->Cell(70, 10, $row['teamname'], 1);
        $pdf->Cell(50, 10, $row['president'], 1);
        $pdf->Cell(50, 10, $row['owner'], 1);
        $pdf->Ln();
    }
