...
if (!empty($searchValue)) {
    ...
    $searchQuery = " AND (eventdate LIKE :searchValue OR year LIKE :searchValue OR event LIKE :searchValue OR eventtype LIKE :searchValue OR team LIKE :searchValue OR 
    game1 LIKE :searchValue OR game2 LIKE :searchValue OR game3 LIKE :searchValue OR game4 LIKE :searchValue OR game5 LIKE :searchValue)";
}
$orderColumn = isset($columns[$orderIndex]) ? $columns[$orderIndex] : 'id';

if($bowlerDeets['bowlerid']){
    if($seasonYear){
        $stmt = $db->prepare("SELECT COUNT(*) as count FROM `bowlerdata` WHERE `bowlerid` = :bowlerid AND `year` = :years $searchQuery");
    }else{
        $stmt = $db->prepare("SELECT COUNT(*) as count FROM `bowlerdata` WHERE `bowlerid` = :bowlerid $searchQuery");
    }
    if (!empty($searchValue)) {
    ...
    if(isset($_POST['exportType'])){
        if($seasonYear){
            $sql = "SELECT * FROM `bowlerdata` WHERE `bowlerid` = :bowlerid AND `year` = :years $searchQuery ORDER BY $orderColumn $orderDir";
        }else{
            $sql = "SELECT * FROM `bowlerdata` WHERE `bowlerid` = :bowlerid $searchQuery ORDER BY $orderColumn $orderDir";
        }
        $stmt = $db->prepare($sql);
        ...
        if($seasonYear){
            $sql = "SELECT * FROM `bowlerdata` WHERE `bowlerid` = :bowlerid AND `year` = :years $searchQuery ORDER BY $orderColumn $orderDir LIMIT :start, :limit";
        }else{
            $sql = "SELECT * FROM `bowlerdata` WHERE `bowlerid` = :bowlerid $searchQuery ORDER BY $orderColumn $orderDir LIMIT :start, :limit";
        }
        $stmt = $db->prepare($sql);
...
function exportCSV($data) {
    ...
    foreach ($data as $row) {
        ...
        $timestamp = strtotime($ymd);
        $dmy = date("m-d-Y", $timestamp);
        $rowRaw = [$dmy, $row['year'], $row['event'], $row['eventtype'],$row['team'],$row['game1'],$row['game2'],$row['game3']];
...
function exportExcel($data) {
    ...
    $sheet->setCellValue('I1', 'Game4');
    $sheet->setCellValue('J1', 'Game5');
    ...
    foreach ($data as $row) {
        ...
        $sheet->setCellValue('D' . $rowNum, $row['eventtype']);
        ...
        $sheet->setCellValue('I' . $rowNum, $row['game4']);
        $sheet->setCellValue('J' . $rowNum, $row['game5']);
...
function exportPDF($data) {
    ...
    foreach ($data as $row) {
        ...
        $pdf->Cell(40, 10, $row['eventtype'], 1);
