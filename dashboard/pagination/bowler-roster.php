...
function getSeasonTourAvg($bowlerID){
    ...
    $avrgseason = $db->prepare("SELECT * FROM `bowlerdataseason` WHERE `bowlerid` = '$bowlerID' AND YEAR(eventdate) BETWEEN '2025' AND '2026' AND year='2025/26' ORDER BY `eventdate` DESC ");
    $avrgseason->execute();
    ...
    $avrgss = 0;
        foreach ($avrgseasonAll as $seasonVal) {
            ...
            $sectValue1 = new DateTime("2025-09-01");
            $sectValue1Formatted = $sectValue1->format('Y-m-d');

            $sectValue2 = new DateTime("2026-09-01");
