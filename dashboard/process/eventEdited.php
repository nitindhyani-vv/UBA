<?php
    ...
    try {
        ...
        $sql = "UPDATE events 
            ...
            $stmt->bindParam(':subevent', $subevent);
            $stmt->bindParam(':eventdate', $eventdate);
            $stmt->bindParam(':eventname', $eventname);
            $stmt->bindParam(':evententry', $entryEvent);
            $stmt->bindParam(':bowlersPerTeam', $bowlersPerTeam);
            $stmt->bindParam(':noOfSquads', $noOfSquads);
            $stmt->bindParam(':phone', $phone);
            $stmt->bindParam(':maxEntryPerSquad', $maxEntryPerSquad);
            $stmt->bindParam(':costPerBowler', $costPerBowler);
            $stmt->bindParam(':eventlocation', $eventlocation);
            $stmt->bindParam(':bowlerRegister', $bowlerRegister);
            $stmt->bindParam(':eventActive', $eventActive);
            $stmt->bindParam(':eventID', $eventID);

            $stmt->execute(); 

            // $statement = $db->prepare("INSERT INTO `events` (`mainevent`, `subevent`, `eventdate`, `evententry`, `perteam`, `teamstructure`, `noOfSquads`, `maxEntryPerSquad`, `costPerBowler`, `location`, `bowlerregister`, `active`)
            // VALUES(:mainevent, :subevent, :eventdate, :evententry, :perteam, :teamstructure, :noOfSquads, :maxEntryPerSquad, :costPerBowler, :eventlocation, :bowlerregister, :active)");
            ...
            header("Location: ".$base_url."/dashboard/editEvent.php");
